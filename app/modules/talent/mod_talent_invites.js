...
function setInfo(message){
  const el = getEl("talentInviteInfo");
  if(el) el.textContent = message || "";
}

function renderItems(items){
  const list = getEl("talentInviteList");
  if(!list) return;
...
export default async function(){
  ...
  const res = await talentGet("/functions/api/talent/my_invites");

  if(!res.ok){
    setInfo("Failed to load invites.");
    renderItems([]);
    showTalentNotice(res?.data?.message || "Failed to load invites.", "error");
    return;
  }
