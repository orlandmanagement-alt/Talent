import { TALENT_ROUTES, findTalentRouteByPath } from "./talent_routes.js";

function ensureNavRoot(){
  let root = document.getElementById("talentPortalNav");
  if(root) return root;

  root = document.createElement("nav");
  root.id = "talentPortalNav";
...
export function renderTalentNav(){
  const root = ensureNavRoot();
  root.innerHTML = "";

  const title = document.createElement("div");
  title.textContent = "Talent Portal Navigation";
  title.style.fontWeight = "700";
  title.style.marginBottom = "10px";
  root.appendChild(title);

  const current = findTalentRouteByPath(location.pathname);

  TALENT_ROUTES.forEach(item => {
